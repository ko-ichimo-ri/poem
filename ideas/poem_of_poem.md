# ポエム・オブ・ポエムへようこそ
ここではとりとめもなく好きなことを好きなように書き散らします。
- 発信したいこと
- 思いついたこと
- 自己紹介的な
...
- 大学院生時代サークルから数学をテーマにした同人誌を販売したことがあります
  ...
  - 自分では覚えてませんが、大学一年生のときからずっと出したいと言っていたらしく、4年後に完成したとき友人に「最終回を見ているようだ」と言われました
...
## 数学やプログラミングの問題集
自作して解いた問題やかつてハマったバグなどで面白いものを共有します。随時更新していきます。
- [数学関連](https://github.com/ko-ichimo-ri/math)
- [プログラミング関連](https://github.com/ko-ichimo-ri/programming_memo)
...
- これも人間行動と同じで「入出力と処理(インタフェースと実装)」で説明できるかも？
  - 合意形成による実体と客体による実体がインタフェースだけみると同一視できる？
    ...
    - じゃあなんで同一視できるんだ？
- 『[制度とは何か──社会科学のための制度論](https://www.keio-up.co.jp/np/detail_contents.do?goods_id=3933)』(フランチェスコ・グァラ (著), Francesco Guala (著), 瀧澤 弘和 (監修, 翻訳),2018,慶應義塾大学出版会)の「制度論的存在」がヒントになりそう？
- 論理的存在の分類(仮)→本当はMECEで分けたい
  1. シミュレーション: 実装が異なるがインタフェースを似せたもの
     - メタバース
     - VR/AR
     - VPN
     - エミュレータ(実装まで論理的存在)
     - シミュレータ(インタフェースのみ論理的存在)
     - 万能チューリングマシン(あるいはチューリング完全なもの)
     - ゲーデル数を引数にとる述語
  1. 所有: 資源の分割を合意形成で行うもの
     - 仮想サーバ
     - 仮想マシン
     - 仮想ネットワーク
     - 法的な所有物
  1. ラベル付け管理: ラベルと実体を分けて管理したもののラベル
     - SNSのアカウント
     - 仮想アドレス(論理アドレス)
  1. 認識的存在: 実体とは異なる認識しかできないもの
     - 延べ人数
     - 再生回数
     - 犯罪者数 
